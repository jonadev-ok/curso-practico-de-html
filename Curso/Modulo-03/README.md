...
<details>
  <summary>Atributos ARIA.</summary>
...
- Atributos de estado y propiedades
    - **aria-hidden="true":** Indica que un elemento debe ser ignorado por las tecnologías de asistencia. Se usa cuando el contenido
    es irrelevante para los lectores de pantalla.
    - **aria-disabled="true":** Indica que un elemento está deshabilitado y no puede ser interactuado.
    - **aria-expanded="true/false":** Indica si un elemento desplegable (como un menú o acordeón) está expandido (true) o colapsado
    (false).
    - **aria-checked="true/false/mixed":** Indica el estado de un control de selección, como un checkbox (true para marcado, false
    para no marcado, y mixed para un estado indeterminado).
    - **aria-selected="true/false":** Indica si un elemento, como una opción de lista, está seleccionado.
    - **aria-label="Texto":** Define una etiqueta personalizada para un elemento que puede no tener texto visible, útil para botones
    de íconos o elementos gráficos.
    - **aria-labelledby="ID":** Asocia el elemento con un ID de otro elemento que actúa como su etiqueta. Similar a label en formularios, 
    pero permite etiquetas más complejas.
    - **aria-describedby="ID":** Asocia un elemento con un ID de otro elemento que proporciona una descripción adicional, útil para
    proporcionar más contexto sobre el uso o estado de un control.
    - **aria-controls="ID":** Indica que el elemento controla otro elemento (por ejemplo, un botón que controla la visibilidad de un
    panel desplegable).
    - **aria-live="polite/assertive":** Define el nivel de prioridad para que los lectores de pantalla anuncien cambios en el contenido
    dinámico. polite: No interrumpir al usuario, esperar hasta que termine su acción. assertive: Interrumpir inmediatamente para anunciar
    el cambio.
    - **aria-role="tablist":** Indica un contenedor de pestañas (tab) que gestiona la interacción entre pestañas y paneles de contenido.
    - **aria-busy="true/false":** Indica que el elemento está cargando o está ocupado. Cuando true, las tecnologías de asistencia
